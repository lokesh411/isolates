...
(async () => {
  let totalTime = 0, totalBatches = 1000, totalCallsInABatch = 20
  for (let i = 0; i < totalBatches; i++) {
    const startTime = Date.now()
    const axiosCalls = Array(totalCallsInABatch).fill(1).map((_, index) => {
      const workerId = uuid.v4()
      const params = { id: workerId, even: true };
      if (index % 2 === 0) {
        params.even = true
      }
      return axios.get(`http://${hostName}:5000/health`, { params, timeout: 4000 }).then(({ data: response }) => {
    ...
    try {
      ...
      console.log(`Batch ${i} failed :: `, error.config.params)
    }
    // await sleep(10)
  }
